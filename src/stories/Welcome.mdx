import { Meta } from "@storybook/blocks";

<Meta title="欢迎" />

<h1>欢迎</h1>
<p>这里是思源开发工具的Storybook，目标是提供更多有价值的开发工具、组件库，赋能更多的思源第三方开发者，提高效率，设计统一</p>

# 组件库
## 原子组件
+ [按钮Button](/?path=/docs/BaseComponent-button--docs)
+ [开关Switch](/?path=/docs/BaseComponent-switch--docs)
+ [输入Input(文字、数字)](/?path=/docs/BaseComponent-input--docs)
+ [文本域Textarea](/?path=/docs/BaseComponent-textarea--docs)
+ [滑块Slider](/?path=/docs/BaseComponent-slider--docs)
+ [选择Select](/?path=/docs/BaseComponent-select--docs)
+ [图标Icon](/?path=/docs/BaseComponent-icon--docs)

## 高级组件
+ [树型结构Tree](/?path=/docs/HighLevel-tree--docs)
+ [侧边Dock](/?path=/docs/HighLevel-dock--docs)（插槽：标题、按钮组、内容）
+ [Dialog](/?path=/story/highlevel-dialog--docs)

## 业务组件
+ Setting
+ Protyle

## 布局组件
+ flex容器

# 技术
+ Storybook
+ Svelte

https://blog.logrocket.com/build-your-own-component-library-svelte/

https://kit.svelte.dev/docs/introduction

https://storybook.js.org/docs/configure/styling-and-css